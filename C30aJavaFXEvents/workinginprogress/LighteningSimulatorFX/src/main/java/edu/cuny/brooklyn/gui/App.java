...
public class App extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		field.addEventFilter(LightningEvent.PLASMA_STRIKE,
				event -> System.out.println("Field filtered strike: " + event.getI() + ", " + event.getJ()));

		field.addEventHandler(LightningEvent.PLASMA_STRIKE,
				event -> System.out.println("Field handled strike: " + event.getI() + ", " + event.getJ()));
	...
	private class StrikeEventHandler implements EventHandler<LightningEvent> {
		...
		public void handle(LightningEvent event) {
			...
			// event.consume(); if event is consumed the handler for the parent node will
			// not be invoked.
		}
	}
	...
	static class LightningEvent extends Event {
		...
		/**
		 * Creates a new {@code LightningEvent} with an event type of
		 * {@code PLASMA_STRIKE}. The source and target of the event is set to
		...
		/**
		 * Construct a new {@code LightningEvent} with the specified event source and
